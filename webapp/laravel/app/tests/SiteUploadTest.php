...
class SiteUploadTest extends TestCase {

	//DEFINE 
	static private $sitename = 'test-upload';
	...
	//Test Install step 3 : upload CMS from blob storage to azure website
	public function testUploadCMS()
	{
		printf('%s%c','Test Install step 3 : upload CMS from blob storage to azure website',10);

		//Clear web site
		SiteUploadTest::deleteFalseSite();

		//Login username=test@nfsite.me password=password
		$loginCreate = SiteUploadTest::loginForCreate();
		...
		$input = array(
				...
				'sitename' => SiteUploadTest::$sitename,
				'domain_id' => '1',
		...
		printf('%s%c','>Request("POST", "/site/install", array(step3, sid)',10);
